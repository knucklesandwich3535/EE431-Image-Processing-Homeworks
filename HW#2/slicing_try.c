// In this try, we tried to do the quadrant assignment on the same image. We sliced the image into 4 pieces and filtered as wanted and reconsturcted it on another array to do the process non-destrucive.
...
for(i = 0 ; i < NR ; i++)
{
	for(j = 0 ; j < NC ; j++)
	{
		...
		{
			temp_1[i][j] = top_right[i][j]; 	// Q2 allocation from top_right array to the temp_1 array
		}
		else if(i > NR / 2 && j < NC / 2)
		{
			temp_1[i][j] = bottom_left[i][j];	// Q3 allocation from bottom_left array to the temp_1 array
		}
		else if(i > NR / 2 && j > NC / 2)
		{
			temp_1[i][j] = bottom_right[i][j];	// Q4 allocation from bottom_right array to the temp_1 array
		}
	}
}
